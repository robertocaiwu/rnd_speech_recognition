...
def find_dependencies():
	...
	status, output = getstatusoutput(["pkg-config", "--libs", "--cflags"])
	...
	if status != 0:
		...
		for libdir in ['/usr/lib', '/usr/lib64', '/usr/lib/x86_64-linux-gnu', '/usr/lib/i386-linux-gnu', '~/usr/lib/lapack']:
			print('%s/libatlas.so.3' % libdir)
		...
		if not os.path.isdir('/usr/lib/atlas-base'):
			raise Exception ('Failed to find atlas includes your system.')

		kw.setdefault('include_dirs', []).append('/usr/lib/atlas-base')
